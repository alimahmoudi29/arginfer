...
/home/amahmoudi/miniconda3/envs/py37/bin/python \
 recomb_breaks.py --weaver_general_path /data/projects/punim0594/Ali/phd/mcmc_out/aw/r1/n10L100K/out5 \
...
def arginfer_recomb_breaks(general_path):
    '''
    count ancestral recomb for all sampled ARGs for one data set
    '''
    ...
    for entry in os.scandir(general_path):
        if (entry.path.endswith(".arg")) and entry.is_file():
            ...
            br.extend(arg.breakpoints(only_ancRec=True, set=False))
    return br

def argweaver_recomb_breaks(general_path, ARGweaver_executable_dir, window):
    '''
    ARGweaver: https://github.com/mdrasmus/argweaver/blob/master/bin/arg-extract-breaks
    ...
    '''
    cmd = os.path.join(ARGweaver_executable_dir, 'arg-extract-breaks ') +\
...
def main(args):
    ...
    weaver_br =argweaver_recomb_breaks(args.weaver_general_path, args.ARGweaver_executable_dir,
                              args.window)
